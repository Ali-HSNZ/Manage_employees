...
const Edit = (props) => {
    ...
    useEffect(()=>{   
        const getAData = async()=>{
            try {
                ...
                setUserData(data)
                setPrevUserData({FirstName : data.FirstName , LastName : data.LastName})
            }catch (error) {}
        }
        if(id){
            getAData()
    ...
    const goBackHandler =()=> {
        ...
        if(type === "userList"){
            ...
            props.history.push(`/userList/all-Profile`)
        }
    }
    ...
    return (  
        <form onSubmit={formik.handleSubmit}>
            <div className={Styles.header}>
                <div>
                    <button type="submit" disabled={!formik.isValid} className={Styles.headerSubmitBtn}>ویرایش</button>
                    <button onClick={goBackHandler} className={Styles.headerGoBackBtn}>برگشت</button>
                </div>
                <p dir="rtl">ویرایش مشخصات کارمند  : </p>
            </div>
